...
import { getPage, navigatePageSimple } from './utils'; 
...
function imageToDataUri(buffer: Buffer, contentType: string): string {
  const du = new Datauri();
  du.format(contentType, buffer);
  return du.content.toString();
}

function cssToDataUri(buffer: Buffer): string {
  const du = new Datauri();
  du.format('text/css', buffer);
  return du.content;
  //return `data:text/css;charset=utf-8,${buffer.toString().replace('"', '\"')}`
}
...
async function scrapeAll(url: string) {
  ...
  console.log('INCOGNITO: ' + page.browserContext().isIncognito());
  ...
  try { 
    ...
    //console.log(page.browser().is)
    await navigatePageSimple(page, url, { waitFor: 9000 });
    ...
    pass0.resources.forEach(resource => { 
      if (resource.type === 'external_css_sheet' || resource.type === 'external_img') {
        externalResources.push(resource); 
      } else {
        embeddedResources.push(resource);
      }
    })

    const retrievedResources = await retreiveResources(page, externalResources);  
    page.browser().disconnect();

    return { pass0, retrievedResources, embeddedResources }
...
scrapeAll(url).then(({ pass0, retrievedResources, embeddedResources }) => {
  ...
  retrievedResources.forEach(resource => {
    ...
    if (inlineExternal) {
      const inlined = imageToDataUri(resource.buffer, ext);
      console.log(`Inlining ${ext} - \n${inlined}\n -`);
      if (resource.type === 'external_css_sheet') {
        el.setAttribute('href', imageToDataUri(resource.buffer, ext)); 
      } else {
        console.log(`Inlining: ${resource.contentType}`);
        el.setAttribute('src', imageToDataUri(resource.buffer, ext));
      }
  ...
  writeFileSync(`${outFolder}/DATA.json`, JSON.stringify({ resources: pass0.resources }, undefined, 2));
